...
`;
  ...
  // Check if this is a text generation model (has Embedding + Transformer layers)
  const hasEmbedding = sortedNodes.some(n => n.data.layerType === 'Embedding');
  const hasTransformerDecoder = sortedNodes.some(n => n.data.layerType === 'TransformerDecoder');
  const isTextGenerationModel = hasEmbedding && hasTransformerDecoder;

  const architecture = sortedNodes.map(n => n.data.label || n.data.layerType).join(' → ');

  // Text generation function for GPT-style models
  const textGenCode = isTextGenerationModel ? `

# ============================================================
# Text Generation (for character-level models)
# ============================================================
def generate_text(model, seed_text, char_to_idx, idx_to_char, max_length=200, temperature=0.8):
    """
    ...
    seq_length = ${inputConfig.seqLength || 64}
    
    # Pad seed text if needed
    current_text = seed_text
    while len(current_text) < seq_length:
    ...
    with torch.no_grad():
        for _ in range(max_length):
            ...
            input_text = current_text[-seq_length:]
            input_ids = [char_to_idx.get(c, 0) for c in input_text]
            input_tensor = torch.tensor([input_ids], dtype=torch.long).to(device)
            
            # Get prediction
            output = model(input_tensor)
            
            # Get logits for last position (or all if sequence output)
...
def demo_generation():
    # Example character vocabulary (replace with your actual vocab)
    ...
    generated = generate_text(model, seed, char_to_idx, idx_to_char, max_length=100, temperature=0.8)
    print(f"Generated: {seed}{generated}")
` : '';
...
class NeuralNetwork(nn.Module):
    ...
    ${isTextGenerationModel ? '\\n    This is a GPT-style autoregressive model for text generation.' : ''}
    """
